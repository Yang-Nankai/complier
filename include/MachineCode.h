...
public:
    ...
    StackMInstrcuton(MachineBlock* p, int op, std::vector<MachineOperand*>srcs,
                MachineOperand* src1, MachineOperand* src2 = nullptr,
                int cond = MachineInstruction::NONE);
    void output();
};
...
private:
    ...
    int cmpCond;
    static int label; //record the label. then condInstruct can jump
public:
    ...
    std::vector<MachineInstruction*>::iterator rbegin() { return inst_list.end(); };
    std::vector<MachineInstruction*>::iterator rend() { return inst_list.begin(); };
