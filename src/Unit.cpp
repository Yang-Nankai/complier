...
void Unit::output() const
{
    for (auto se : global_list) {
        if(se->getType()->isInt())
            fprintf(yyout, "%s = global %s %d, align 4\n", se->toStr().c_str(),se->getType()->toStr().c_str(),((IdentifierSymbolEntry*)se)->getIntValue());
        else if(se->getType()->isFloat())
            fprintf(yyout, "%s = global %s %f, align 4\n", se->toStr().c_str(),se->getType()->toStr().c_str(),((IdentifierSymbolEntry*)se)->getFloatValue());
    }

    for (auto& func : func_list)
        func->output();
    for (auto se : declare_list) {
        FuncType* type = (FuncType*)(se->getType());
