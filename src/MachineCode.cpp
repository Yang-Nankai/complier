...
void BranchMInstruction::output()
{
    ...
    case B:
        fprintf(yyout, "\tb");
        PrintCond();  //here the printcond can be used!! beq??or else!
        fprintf(yyout, " ");
        this->use_list[0]->output();
        fprintf(yyout, "\n");
        break;
    ...
    int cond)
{
    ...
    this->op = -1;
    this->cond = cond;
    p->setCmpCond(cond); //set cmpCond, the last instruction must cond
    this->use_list.push_back(src1);
    this->use_list.push_back(src2);
    src1->setParent(this);
    src2->setParent(this);
...
{ 
    ...
    this->stack_size = 0;
    this->paramsNum = ((FuncType*)(sym_ptr->getType()))->getParamsSe().size();
};
...
void MachineFunction::output()
{
    ...
    *  4. Allocate stack space for local variable */
    ...
    int count = 0;
    //output all blocks of funtion!
    for(auto iter : block_list) {
        iter->output();
        count += iter->getSize();
        //if the count > 160 but there is not use????
        if(count > 160) {
            fprintf(yyout, "\tb .F%d\n", parent->getN());
            fprintf(yyout, ".LTORG\n");
            parent->printGlobal();
...
{
    bool first = true;  //set the r3 used?
    int offset = (parent->getSavedRegs().size() + 2) * 4;  //get the offset
    int count = 0;
    if(!inst_list.empty()) {
        fprintf(yyout, ".L%d:\n", this->no);
        for(auto it = inst_list.begin(); it != inst_list.end(); it++) {
            ...
            if(parent->getParamsNum() > 4 && (*it)->isStore()) {
                MachineOperand* opr = (*it)->getUse()[0];
                //本程序中主要是用r4来进行普通的赋值load之类的，需要进行注意后期是否进行改变！！！
                if(opr->isReg() && opr->getReg()==3) { 
                    if(first) {
                        first = false;
                    }else {
                        auto fp = new MachineOperand(MachineOperand::REG, 11);
            ...
            (*it)->output();
            count++;
            //here have something problems!
            if(count % 500 == 0) {
                fprintf(yyout, "\tb .B%d\n", label);
                fprintf(yyout, ".LTORG\n");
                parent->getParent()->printGlobal();
                fprintf(yyout, ".B%d:\b", label++);
            }
        }
    }
...
void MachineUnit::PrintGlobalDecl()
{
    ...
    for(long unsigned int i = 0; i < global_list.size(); i++) {
        ...
        if(se->isConstant()){ //here is different??
            ...
            //not array
            if(se->getType()->isInt()){
                fprintf(yyout, "\t.word %d\n", se->getIntValue()); //here consider the Int
            }else if(se->getType()->isFloat()) {
                fprintf(yyout, "\t.word %d\n", se->getFloatValue()); //here somthing wrong!
            }
        }
    }
