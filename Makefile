...
TESTCASE6 = $(shell find $(TEST_PATH6) -name "*.sy")
...
OUTPUT_LAB6 = $(addsuffix .ll, $(basename $(TESTCASE6)))
...
.phony:all app run gdb testlab4 testlab5 testlab6 test clean clean-all clean-test clean-app llvmir run1
...
run1:app
	@$(BINARY) -o example.ll -i example.sy
	clang example.ll sysyruntimelibrary/sylib.c -o example
	./example
	echo $?
...
testlab6:app
	@success=0
	@for file in $(sort $(TESTCASE6))
	do
		AST=$${file%.*}.ast
		LOG=$${file%.*}.log
		echo "$${file}"
		@$(BINARY) -a $${file} -o $${AST} -i 2>$${LOG}
		echo "\n"
	done
	echo "done!"
	:
